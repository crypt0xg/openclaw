...
- OpenClaw derives context-window preflight thresholds from the detected effective window. It warns below 20% of that window with an **8k** floor and blocks below 10% with a **4k** floor. If you hit that preflight, raise the server/model context limit or choose a larger model.
